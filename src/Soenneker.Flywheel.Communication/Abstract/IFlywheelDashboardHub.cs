...
public interface IFlywheelDashboardHub
{
    ...
    /// <summary>Subscribes to a search page with status exclusions applied before pagination.</summary>
    Task SubscribeFilteredBoard(int version, string? query, int offset, int count, bool summary, DateTimeOffset? startAt, DateTimeOffset? endAt, string? excludedStates);
