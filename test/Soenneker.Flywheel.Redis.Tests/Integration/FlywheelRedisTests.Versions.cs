...
public sealed partial class FlywheelRedisTests
{
    ...
    public Task VersionSubmissionIsAtomicAndSurvivesRetention() => WithStore(async (store, db, ns) =>
    {
        ...
        string[] ids = await Task.WhenAll(Enumerable.Range(0, 20).Select(i =>
            (i % 2 == 0 ? store : other).EnqueueForCurrentInstance(Request(), "build-2", "new")));
    ...
    public Task VersionClientAndExecutorUseHostingApplicationVersion() => WithStore(async store =>
    {
        ...
        string id = await current.GetRequiredService<IJobClient>()
            .EnqueueForCurrentInstance(FlywheelJobs.IntegrationJobs_Run, new TestPayload("first"));
        string duplicate = await peer.GetRequiredService<IJobClient>()
            .EnqueueForCurrentInstance(FlywheelJobs.IntegrationJobs_Run, new TestPayload("second"));
        Check(id != duplicate, "Distinct instances shared a startup job");
        Check(await current.GetRequiredService<IJobClient>().EnqueueForCurrentInstance(FlywheelJobs.IntegrationJobs_Run, new TestPayload("ignored")) == id, "Same instance duplicated job");
        ...
        Check(await current.GetRequiredService<IJobExecutor>().RunOnce(default), "Original instance did not execute its job");
