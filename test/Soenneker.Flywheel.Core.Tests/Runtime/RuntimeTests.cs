...
public sealed partial class RuntimeTests
{
    ...
    {
        var store = new StubStore { Status = Enums.LeaseStatus.Lost, StalledRenewal = stalledRenewal };
    ...
    {
        await using ServiceProvider services = new ServiceCollection().BuildServiceProvider();
        foreach (LeaseStatus status in new[] { Enums.LeaseStatus.Renewed, Enums.LeaseStatus.CancellationRequested })
        {
            ...
            using var deadline = new CancellationTokenSource(TimeSpan.FromSeconds(5));
            await executor.RunOnce(deadline.Token);
            JobOutcome expected = status == Enums.LeaseStatus.Renewed ? Enums.JobOutcome.Failed : Enums.JobOutcome.Cancelled;
