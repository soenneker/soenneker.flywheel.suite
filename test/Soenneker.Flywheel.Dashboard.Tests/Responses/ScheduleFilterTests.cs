...
using SchedulesPage = Soenneker.Flywheel.Dashboard.Pages.Schedules.Schedules;
...
public sealed class ScheduleFilterTests
{
    ...
    public void RecurringSearchReusesResultsAndInvalidatesOnQueryOrSnapshotChanges()
    {
        ...
        IReadOnlyList<RecurringScheduleView> filtered = Read();
        ...
        void Set(string name, object? value) => typeof(SchedulesPage).GetField(name, BindingFlags.Instance | BindingFlags.NonPublic)!.SetValue(board, value);
        IReadOnlyList<RecurringScheduleView> Read() => (IReadOnlyList<RecurringScheduleView>)typeof(SchedulesPage)
