...
public sealed class DemoTour(IJobClient client, IJobStore store, ILogger<DemoTour> logger)
{
    public async Task<DemoRun> Run(string? batchId = null, CancellationToken cancellationToken = default)
    {
        ...
        await Add("15-second work: completes successfully", FlywheelJobs.LongRunningJobs_FifteenSeconds,
            new WorkRequest(15), "fifteen-seconds", new JobPolicy { MaxAttempts = 1, Timeout = TimeSpan.FromSeconds(30) });
        await client.Recurring("demo-fifteen-seconds-v1", FlywheelJobs.LongRunningJobs_FifteenSeconds,
            new WorkRequest(15), TimeSpan.FromMinutes(2), cancellationToken: cancellationToken);
