...
public sealed class LongRunningJobs(ILogger<LongRunningJobs> logger, IJobProgress progress)
{
    [FlywheelJob("demo.work.fifteen-seconds.v1")]
    public Task FifteenSeconds(WorkRequest request, CancellationToken cancellationToken) =>
        Work(new WorkRequest(15), cancellationToken);
    ...
    private async Task Work(WorkRequest request, CancellationToken cancellationToken)
    {
        ...
        await progress.Report(0, "Starting work", cancellationToken);
        try
        {
            for (var elapsed = 0; elapsed < request.Seconds; elapsed++)
            {
                await Task.Delay(1000, cancellationToken);
                await progress.Report((elapsed + 1) * 100d / request.Seconds, $"{elapsed + 1}/{request.Seconds} seconds", cancellationToken);
